...
(function(ªG) {
/*! Proon 0.0.5 //// MIT Licence //// http://proon.richplastow.com/ */
var Proon, SomeClass, Tudor, objectSerializer, tudor, ª, ªA, ªB, ªC, ªE, ªF, ªN, ªO, ªR, ªS, ªU, ªV, ªX, ªex, ªhas, ªinsert, ªis, ªisU, ªredefine, ªtype, ªuid,
...
Proon = (function() {
  ...
  Proon.prototype["delete"] = function(node) {
    var M, curr, i, j, k, len, len1, maxLevels, name, path, str;
...
tudor.add([
  "03-1 Proon `delete()` object", tudor.is, "The method is the expected type", function() {
    ...
    return objectSerializer(proon.object);
  }, "Delete a top-level leaf-node, `proon.delete({ name:'a' })`", '{}', function(proon) {
    proon["delete"]({
      name: 'a'
    });
    return objectSerializer(proon.object);
    ...
    return objectSerializer(proon.object);
  }, "Delete first second-level leaf-node", '{"a":{"c":"two"}}', function(proon) {
    proon["delete"]({
      path: ['a'],
      name: 'b'
    });
    return objectSerializer(proon.object);
  }, "Delete second second-level leaf-node", '{}', function(proon) {
    proon["delete"]({
      path: ['a'],
      name: 'c'
    });
    ...
    return objectSerializer(proon.object);
  }, "Add a fifth-level leaf-node, and insert a sub-tree in the third-level", '{"a":{"b":{"c":{"d":{"e":{"f":"deep!"}},"g":"middle","p":{"q":{"r":"side"}}}}}}', function(proon) {
    proon.add({
      path: ['a', 'b', 'c', 'd', 'e'],
      name: 'f',
      content: 'deep!'
