...
(function(ªG) {
/*! Proon 0.0.7 //// MIT Licence //// http://proon.richplastow.com/ */
var Proon, SomeClass, Tudor, fsSerializer, objectSerializer, storageSerializer, tudor, ª, ªA, ªB, ªC, ªE, ªF, ªN, ªO, ªR, ªS, ªU, ªV, ªX, ªex, ªhas, ªinsert, ªis, ªisU, ªredefine, ªtype, ªuid,
...
Proon = (function() {
  ...
  Proon.prototype.add = function(node) {
    var M, content, curr, e, fsE1, fsE2, i, j, key, len, len1, len2, len3, len4, len5, m, maxLen, maxLevels, name, nameRx, o, path, pathRx, q, r, real, ref, ref1, rel, stat, str, u, v, value;
    ...
    if (this.fs) {
      ...
      if (real.length === path.length) {
        ...
        try {
          ...
          fsE2 = _error;
          if ('ENOENT' !== fsE2.code) {
            throw Error("" + M + fsE2.code + " checking `node.name` '" + name + "'");
          }
        }
        if (!fsE2.code) {
          if (stat.isFile()) {
    ...
    if (this.fs) {
      rel = rel.replace(/\/[^\/]+$/, '');
      if (real.length !== path.length) {
        for (i = v = ref = real.length, ref1 = path.length - 1; ref <= ref1 ? v <= ref1 : v >= ref1; i = ref <= ref1 ? ++v : --v) {
          rel += '/' + path[i];
          try {
...
fsSerializer = function(fs, pwd) {
  var item, j, l, len, len1, m, out, ref, stat, sub, subs;
  if (pwd == null) {
    pwd = '';
  }
...
tudor.add([
  ...
  }, "`proon.add()` filesytem usage", tudor.equal, "`proon.filesytem` contains the expected key/value pairs", "a\na/b\na/b/c\nd", function(proon) {
    return fsSerializer(proon.fs, proon.pwd);
