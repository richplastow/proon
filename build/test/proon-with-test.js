...
(function(ªG) {
/*! Proon 0.0.14 //// MIT Licence //// http://proon.richplastow.com/ */
var Node, Proon, SomeClass, Tudor, storageSerializer, tudor, ª, ªA, ªB, ªC, ªD, ªE, ªF, ªN, ªO, ªR, ªS, ªU, ªV, ªX, ªex, ªhas, ªinsert, ªis, ªisU, ªredefine, ªtype, ªuid,
...
Proon = (function() {
  ...
  function Proon(config) {
    ...
    } else if (ªF !== ªtype(config.storage.setItem)) {
    ...
    if (ªU === typeof config.dom) {
      ...
      if (-1 === ['htmldocument', ªD, ªO].indexOf(tdom)) {
        throw TypeError(M + "Optional `config.dom` is " + (ªtype(config.dom)) + " not 'htmldocument', " + ªD + " or " + ªO);
      } else {
        this.dom = config.dom;
      }
    }
    ...
    if (ªU === typeof config.root) {
      this.root = '_';
    } else if (ªS !== ªtype(config.root)) {
      throw TypeError(M + "Optional `config.root` is " + (ªtype(config.root)) + " not string");
    } else if (!rootRx.test(config.root)) {
      throw RangeError(M + "Optional `config.root` fails " + rootRx);
    } else {
  ...
  Proon.prototype.add = function(node) {
    var M, aa, content, curr, e, el, i, id, j, key, len1, len2, len3, len4, len5, len6, len7, len8, m, maxLen, maxLevels, name, nameRx, parent, path, pathRx, q, r, real, ref, ref1, rel, stat, str, u, value, w, y, z;
    ...
    if (this.object) {
      curr = this.object;
      for (i = m = 0, len2 = path.length; m < len2; i = ++m) {
        str = path[i];
        curr = curr[str];
        if (ªS === typeof curr) {
    ...
    if (this.dom) {
      ...
      for (i = u = 0, len5 = path.length; u < len5; i = ++u) {
        ...
        if ('I' === el.tagName) {
          throw RangeError(M + "`node.path[" + i + "]` '" + str + "' is already a dom leaf-element");
        } else if ('B' !== el.tagName) {
          throw RangeError(M + "`node.name[" + i + "]` '" + str + "' is already a non-Proon dom element");
        }
      }
      if (el) {
        el = this.dom.getElementById(id + '_' + name);
        if (el) {
          if ('I' === el.tagName) {
            throw RangeError(M + "`node.name` '" + name + "' is already a dom leaf-element");
          }
          if ('B' === el.tagName) {
            throw RangeError(M + "`node.name` '" + name + "' is already a dom branch-element");
          }
          throw RangeError(M + "`node.name` '" + name + "' is already a non-Proon dom element");
        }
      }
    }
    if (this.object) {
      curr = this.object;
      for (i = w = 0, len6 = path.length; w < len6; i = ++w) {
        str = path[i];
    ...
    if (this.storage) {
      key = '/';
      for (i = y = 0, len7 = path.length; y < len7; i = ++y) {
        str = path[i];
    ...
    if (this.fs) {
      rel = rel.replace(/\/[^\/]+$/, '');
      if (real.length !== path.length) {
        for (i = z = ref = real.length, ref1 = path.length - 1; ref <= ref1 ? z <= ref1 : z >= ref1; i = ref <= ref1 ? ++z : --z) {
          rel += '/' + path[i];
          try {
    ...
    if (this.dom) {
      id = this.root;
      el = this.dom.getElementById(id);
      for (i = aa = 0, len8 = path.length; aa < len8; i = ++aa) {
        str = path[i];
        parent = el;
        id += '_' + str;
        el = this.dom.getElementById(id);
        if (!el) {
          el = this.dom.createElement('b');
          el.setAttribute('id', id);
          parent.appendChild(el);
        }
      }
      parent = el;
      el = this.dom.createElement('i');
  ...
  Proon.prototype["delete"] = function(node) {
    var M, aa, curr, e, el, i, id, items, j, k, key, l, len1, len2, len3, len4, len5, len6, len7, m, maxLevels, name, nameRx, parent, path, pathRx, pid, q, r, ref, ref1, rel, stat, str, subs, u, value, w, y, z;
    ...
    if (this.dom) {
      ...
      for (i = u = 0, len5 = path.length; u < len5; i = ++u) {
        ...
        el = this.dom.getElementById(id);
        if (!el) {
          throw RangeError(M + "`node.path[" + i + "]` '" + str + "' is not found in the dom");
        }
        if ('I' === el.tagName) {
          throw RangeError(M + "`node.path[" + i + "]` '" + str + "' is a dom leaf- not branch-element");
        } else if ('B' !== el.tagName) {
          throw RangeError(M + "`node.name[" + i + "]` '" + str + "' is a non-Proon dom element");
        }
      }
      if (name) {
        el = this.dom.getElementById(id + '_' + name);
        if (!el) {
          throw RangeError(M + "`node.name` '" + name + "' is not found in the dom");
        }
        if ('B' === el.tagName) {
          throw RangeError(M + "`node.name` '" + name + "' is a dom branch- not leaf-element");
        }
        if ('I' !== el.tagName) {
          throw RangeError(M + "`node.name` '" + name + "' is a non-Proon dom element");
        }
      }
    }
    if (this.object) {
    ...
    if (this.storage) {
      ...
      if (name) {
        ...
        l = key.length;
        subs = [];
        for (i = y = 0, ref = this.storage.length - 1; 0 <= ref ? y <= ref : y >= ref; i = 0 <= ref ? ++y : --y) {
          if ((k = this.storage.key(i)) && k.substr(0, l) === key) {
            subs.push(k);
          }
        }
        for (z = 0, len7 = subs.length; z < len7; z++) {
          k = subs[z];
      ...
      while (key) {
        l = key.length;
        for (i = aa = 0, ref1 = this.storage.length - 1; 0 <= ref1 ? aa <= ref1 : aa >= ref1; i = 0 <= ref1 ? ++aa : --aa) {
    ...
    if (this.dom) {
      id = path.length ? this.root + '_' + path.join('_') : this.root;
      if (name) {
        parent = this.dom.getElementById(id);
        el = this.dom.getElementById(id + '_' + name);
      ...
      while (id.length > this.root.length) {
        el = this.dom.getElementById(id);
        if (0 !== el.childNodes.length) {
          break;
        }
        pid = id.replace(/_[^_]+$/, '');
        parent = this.dom.getElementById(pid);
        parent.removeChild(el);
        id = pid;
      }
    }
  ...
  Proon.prototype._fsSerializer = function(pwd) {
    var M, item, items, j, len1, len2, m, ref, stat, sub, subs;
    if (pwd == null) {
      pwd = this.pwd;
    }
  ...
  Proon.prototype._fsClear = function(start) {
    var M, e, item, items, j, len1, ref, results, stat;
    if (start == null) {
      start = this.pwd;
    }
  ...
  Proon.prototype._domSerializer = function(root) {
    var M, child, childId, childNode, childNodes, children, el, j, l, len1, len2, len3, m, name, out, q, ref, sub, subs;
    if (root == null) {
      root = this.root;
    }
    M = "/proon/src/Proon.litcoffee _domSerializer()\n  ";
    el = this.dom.getElementById(root);
    ...
    out = [];
    for (m = 0, len2 = children.length; m < len2; m++) {
      child = children[m];
      childId = child.getAttribute('id');
      name = childId.substr(l + 1);
      if ('I' === child.tagName) {
        out.push(name + (child.innerHTML ? ' ' + child.innerHTML : ''));
      } else if ('B' === child.tagName) {
        out.push(name);
        subs = this._domSerializer(childId);
        if ('[EMPTY]' === subs) {
          continue;
        }
        ref = subs.split('\n');
  ...
  Proon.prototype._domClear = function(start) {
    var M, child, childNodes, i, j, len1, parent, results;
    if (start == null) {
      start = this.root;
    }
    M = "/proon/src/Proon.litcoffee _domClear()\n  ";
    parent = this.dom.getElementById(start);
    childNodes = [];
    i = parent.childNodes.length;
    while (i--) {
      childNodes.unshift(parent.childNodes[i]);
    }
    results = [];
    for (j = 0, len1 = childNodes.length; j < len1; j++) {
      child = childNodes[j];
      if ('B' === child.tagName) {
        this._domClear(child.getAttribute('id'));
      }
      results.push(parent.removeChild(child));
    }
    return results;
  };
...
if (!Object.keys) {
  Object.keys = (function() {
    ...
    return function(o) {
      ...
      if (hasDontEnumBug) {
        for (i = j = 0, ref = dontEnumsLength; 0 <= ref ? j <= ref : j >= ref; i = 0 <= ref ? ++j : --j) {
...
ªtype = function(x) {
  ...
  if (ªS !== tx && !x.nodeName && x.constructor !== Array && /function/i.test('' + x)) {
    return ªF;
  }
...
Node = (function() {
  ...
  function Node(config) {
    ...
    M = "/domlet/src/Node/Node-base.litcoffee Node()\n  ";
    this.childNodes = [];
  ...
  Node.prototype.appendChild = function(aChild) {
    var M, length, ref;
    M = "/domlet/src/Node/Node-base.litcoffee Node.appendChild()\n  ";
  ...
  Node.prototype.insertBefore = function(newNode, referenceNode) {
    var M, i, ref;
    M = "/domlet/src/Node/Node-base.litcoffee Node.insertBefore()\n  ";
  ...
  Node.prototype.removeChild = function(child) {
    var M, i, ref, ref1;
    M = "/domlet/src/Node/Node-base.litcoffee Node.removeChild()\n  ";
...
Node.HTMLDocument = (function(superClass) {
  ...
  function HTMLDocument(tagName) {
    var M;
    M = "/domlet/src/Node/Node.HTMLDocument.litcoffee Node.HTMLDocument()\n  ";
    HTMLDocument.__super__.constructor.call(this);
    this.body = new Node.HTMLElement(this, 'body');
    this.idLut = {};
  }

  HTMLDocument.prototype.createElement = function(tagName) {
    return new Node.HTMLElement(this, tagName);
  };
...
Node.HTMLElement = (function(superClass) {
  ...
  function HTMLElement(doc, tagName) {
    var M;
    M = "/domlet/src/Node/Node.HTMLElement.litcoffee Node.HTMLElement()\n  ";
  ...
  HTMLElement.prototype.setAttribute = function(name, value) {
    ...
    if ('id' === name) {
      this.document.idLut[value] = this;
    }
    return void 0;
  };
...
Tudor = (function() {
  ...
  Tudor.prototype["do"] = function() {
    var actual, art, artFail, artPass, article, e, error, expect, heading, j, job, len1, len2, len3, m, mock, mockFail, pge, pgeFail, pgePass, q, ref, ref1, ref2, result, runner, sec, secFail, secPass, section, summary;
    pge = [];
    mock = [];
...
tudor.add([
  "02-4 Proon `add()` dom", tudor.is, "(Set up a Proon instance with dom)", function() {
    var dom, domMock, root;
    domMock = new Node.HTMLDocument;
  ...
  }, "`proon.add()` records to dom without error", ªO, function(proon) {
    return proon.add({
      name: 'c',
      path: ['a', 'b']
    }).add({
  ...
  }, "`proon.add()` dom exceptions", tudor["throw"], "`node.name` must not already be a branch-node", "/proon/src/Proon.litcoffee Proon:add()\n  `node.name` 'a' is already a dom branch-element", function(proon) {
    return proon.add({
      name: 'a'
    });
  }, "`node.name` must not already be a top-level leaf-node", "/proon/src/Proon.litcoffee Proon:add()\n  `node.name` 'd' is already a dom leaf-element", function(proon) {
    return proon.add({
      name: 'd'
    });
  }, "`node.name` must not already be a leaf-node", "/proon/src/Proon.litcoffee Proon:add()\n  `node.name` 'c' is already a dom leaf-element", function(proon) {
    return proon.add({
      name: 'c',
      path: ['a', 'b']
    });
  }, "If set, `node.path` must not overwrite a leaf-node", "/proon/src/Proon.litcoffee Proon:add()\n  `node.path[2]` 'c' is already a dom leaf-element", function(proon) {
    return proon.add({
      name: 'nope',
      path: ['a', 'b', 'c']
    });
  ...
  }, "`proon.add()` cleanup", tudor.equal, "`proon.dom` as expected before cleanup", "a\na_b\na_b_c\nd ok", function(proon) {
    return proon._domSerializer();
  }, "(Clean up dom)", void 0, function(proon) {
    var root;
...
tudor.add([
  "03-4 Proon `delete()` dom", tudor.is, "(Set up a Proon instance with dom)", function() {
    var dom, domMock, root;
    domMock = new Node.HTMLDocument;
    if (ªU === typeof window || ªO !== typeof window.document || ªO !== typeof window.document.body) {
      dom = domMock;
  ...
  }, "`proon.delete()` works on dom without error", ªO, function(proon) {
    proon.add({
      name: 'a'
    });
    return proon["delete"]({
  ...
  }, "`proon.delete()` dom usage", tudor.equal, "`proon.dom` is empty after the previous test", '[EMPTY]', function(proon) {
    return proon._domSerializer();
  }, "Add a top-level leaf-node, `proon.add({ name:'a', content:'ok' })`", 'a ok', function(proon) {
    proon.add({
      name: 'a',
    ...
    return proon._domSerializer();
  }, "Add a fifth-level leaf-node", "a\na_b\na_b_c\na_b_c_d\na_b_c_d_e\na_b_c_d_e_f deep!", function(proon) {
    proon.add({
      path: ['a', 'b', 'c', 'd', 'e'],
      name: 'f',
      content: 'deep!'
    ...
    return proon._domSerializer();
  }, "Delete the third-level leaf-node, but not the fifth-level leaf-node", "a\na_b\na_b_c\na_b_c_d\na_b_c_d_e\na_b_c_d_e_f returned!", function(proon) {
    proon["delete"]({
      path: ['a', 'b', 'c'],
      name: 'g'
    });
    return proon._domSerializer();
  }, "Spike dom with an unexpected item", "a\na_b\na_b_c\na_b_c_d\na_b_c_d_e\na_b_c_d_e_f returned!\noops How unexpected", function(proon) {
    var oops, rootEl;
    oops = proon.dom.createElement('i');
    oops.setAttribute('id', proon.root + '_oops');
    oops.innerHTML = 'How unexpected';
    rootEl = proon.dom.getElementById(proon.root);
    rootEl.appendChild(oops);
    return proon._domSerializer();
  }, "Delete the second-level branch-node", 'oops How unexpected', function(proon) {
    proon["delete"]({
      path: ['a', 'b']
    ...
    return proon._domSerializer();
  }, "`proon.delete()` dom exceptions", tudor["throw"], "The `node` argument is not optional", "/proon/src/Proon.litcoffee Proon:delete()\n  `node` is undefined not object", function(proon) {
    return proon["delete"]();
  }, "The `node` argument must not be an array", "/proon/src/Proon.litcoffee Proon:delete()\n  `node` is array not object", function(proon) {
    return proon["delete"]([]);
  }, "If set, `node.name` must not be an array", "/proon/src/Proon.litcoffee Proon:delete()\n  `node.name` is array not string", function(proon) {
  ...
  }, "If set, `node.name` must exist", "/proon/src/Proon.litcoffee Proon:delete()\n  `node.name` 'x' is not found in the dom", function(proon) {
    proon.add({
      name: 'c',
      path: ['a', 'b']
    });
  ...
  }, "If set, `node.name` must not refer to a branch-node", "/proon/src/Proon.litcoffee Proon:delete()\n  `node.name` 'a' is a dom branch- not leaf-element", function(proon) {
    return proon["delete"]({
      name: 'a'
    });
  }, "If set, `node.name` must not be '__'", "/proon/src/Proon.litcoffee Proon:delete()\n  `node.name` fails /^[a-z][-a-z0-9]{0,23}$/", function(proon) {
  ...
  }, "If set, `node.path` must exist", "/proon/src/Proon.litcoffee Proon:delete()\n  `node.path[1]` 'x' is not found in the dom", function(proon) {
    return proon["delete"]({
      path: ['a', 'x'],
      name: 'c'
    });
  }, "If set, `node.path` must node refer to a leaf-node", "/proon/src/Proon.litcoffee Proon:delete()\n  `node.path[2]` 'c' is a dom leaf- not branch-element", function(proon) {
    return proon["delete"]({
      path: ['a', 'b', 'c']
    });
  }, "`proon.delete()` cleanup", tudor.equal, "`proon.dom` as expected before cleanup", "a\na_b\na_b_c", function(proon) {
    return proon._domSerializer();
  }, "(Clean up dom)", void 0, function(proon) {
    var root;
    root = proon.dom.getElementById('_tmp');
