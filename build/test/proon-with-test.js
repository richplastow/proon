...
(function(ªG) {
/*! Proon 0.0.13 //// MIT Licence //// http://proon.richplastow.com/ */
...
Proon = (function() {
  ...
  function Proon(config) {
    ...
    if (ªU === typeof config.dom) {
      ...
      if ('htmldocument' !== tdom && ªO !== tdom) {
        throw TypeError(M + "Optional `config.dom` is " + (ªtype(config.dom)) + " not " + ªO + " or " + ªD);
      } else {
        this.dom = config.dom;
      }
    }
    ...
    if (this.dom) {
      ...
      if (el) {
        ...
        if (el) {
          ...
          throw RangeError(M + "`node.name` '" + name + "' is already a non-Proon dom element " + el.tagName);
        }
      }
    }
    if (this.object) {
  ...
  Proon.prototype["delete"] = function(node) {
    var M, curr, e, i, items, j, k, key, l, len1, len2, len3, len4, len5, len6, m, maxLevels, name, nameRx, path, pathRx, q, r, ref, ref1, rel, stat, str, subs, u, value, w, y, z;
    ...
    if (this.storage) {
      ...
      if (name) {
        ...
        for (y = 0, len6 = subs.length; y < len6; y++) {
          k = subs[y];
